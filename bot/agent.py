...
from bot.gemini_client import create_chat_with_fallback
...
_TOOL_DECLARATIONS = [
    ...
    types.FunctionDeclaration(
        ...
        parameters=types.Schema(
            type="OBJECT",
            properties={
                "archivo_id": types.Schema(type="STRING", description="El campo 'id' (UUID) del registro en archivos_drive, devuelto por buscar_comprobantes. NO usar el drive_file_id ni la URL."),
...
11. Los PDFs de facturas y comprobantes de los servicios fijos (Edenor, MetroGAS, AySA,
    Personal, expensas) los archiva el sistema solo, de forma determinística, sin pasar por vos.
    No tenés herramienta para subir archivos: si el usuario te pide subir uno, decile que
    se lo mande directo al chat y el sistema lo archiva.
...
14. Si el usuario pide eliminar un comprobante, usá `eliminar_comprobante_drive` con el archivo_id.
